...
export default function Mapper({ data, isForPost, deletePost }) {
        ...
        isForPost ? (
          data.map((post) => (
            <div
              key={post.id}
              className={`post ${
                hoveredPostId && hoveredPostId !== post.id
                  ? "opacity-50"
                  : "opacity-100"
              }`}
            ...
            >
              <h2
                className="font-bold mb-5 hover:cursor-pointer"
          ...
          data.map((comment) => (
            <div key={comment.id} className="comment">
              <p>
                <span className="text-purple-400">Name:</span> {comment.name}{" "}
                <span>&lt;{comment.email}&gt;</span>
              </p>
              <p>
                <span className="text-purple-400">Body: </span> {comment.body}
              </p>
            </div>
          ))
        )
      ) : (
        <p>Keine {isForPost ? `Posts` : `Kommantare`} vorhanden</p>
      )}
    </>
  );
}
